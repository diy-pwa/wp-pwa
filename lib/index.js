...
export default class {
    ...
    async dev() {
        ...
        const app = express();
        app.use(fileUpload());
        app.use(compression({ filter: (_, res) =>{
            ...
            try {
                ...
                Object.keys(resp.resp.headers).forEach((key) => {
                    ...
                    if (key != "x-frame-options") {
                        if (key == "content-type" && value == "application-octet-stream" && req.url.match(/.webp$/)){
                            res.setHeader(key, "image/webp");
    ...
    getRelative(canonical, sPath){
        return canonical.replace(/(http|https):\/\/(localhost|127.0.0.1|playground\.wordpress\.net\/scope):\d+\.?\d*\/*/g, sPath ? `/${sPath}/` : "/");
    }
