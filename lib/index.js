...
`);
    ...
    async dev() {
        const sUrl = new URL(import.meta.url.replace("index.js", "worker.js"));
        const worker = new Worker(sUrl);
        // Store active tasks: id -> { resolve, reject }
        const pendingTasks = new Map();

        // Global listener for ALL worker messages
        worker.on('message', ({ id, result, error }) => {
            ...
            pendingTasks.delete(id); // Clean up
        });

        /**
         * Awaitable helper that tracks task IDs
         */
        function sendTask(payload) {
            return new Promise((resolve, reject) => {
                const id = crypto.randomUUID(); // Unique ID for this specific call
        ...
        app.use(`/wp-includes`, express.static("wp-includes"));
    ...
    async setup() {
        if (!fs.existsSync("wp-admin")) {
            let sFilter = "wp-content";
            if (!fs.existsSync("wp-content")) {
                sFilter = "don't want to filter anything"
            }
            const oZip = new Zip({
        ...
        if (!fs.existsSync("wp-config.php")) {
            let sConfig = (await fs.promises.readFile(`wp-config-sample.php`)).toString();
            sConfig = `<?php
define( 'WP_DEBUG_DISPLAY', false );
define( 'WP_DEBUG_LOG', true );     
define('USE_FETCH_FOR_REQUESTS',true);
define( 'CONCATENATE_SCRIPTS', false );
define( 'WP_MEMORY_LIMIT', '2G' );?>` + sConfig;
            await fs.promises.writeFile(`wp-config.php`, sConfig);
...
ini_set('error_log', $log_file);?>`);
        ...
        if (!fs.existsSync("wp-content/mu-plugins/sqlite-database-integration") &&
            ...
            const oZip = new Zip({
                url: "https://github.com/WordPress/sqlite-database-integration/archive/refs/heads/main.zip",
        ...
        if (!fs.existsSync("wp-content/mu-plugins/one-click-child-theme") &&
            ...
            const oZip = new Zip({
                url: "https://github.com/diy-pwa/one-click-child-theme/archive/refs/heads/main.zip",
    ...
    async buildFile(oUrl, sOutPath) {
        ...
        await fs.promises.writeFile(`dist/${sPath}index.html`, this.getRelativeWithPath(sContents, sOutPath));
    }
    ...
    async getPhpInstance() {
        if (!this.php) {
            ...
            this.php = new PHP(await loadNodeRuntime('8.3', {
                emscriptenOptions: {
                    processId: process.pid // Use a numeric ID if possible
                }
            }));
            await useHostFilesystem(this.php);
        }
        return this.php;
    }
    ...
    async doRequest(sPathname) {
        ...
        const scriptPath = `${process.cwd()}/index.php`;
        return (php.run({
    ...
    getRelativeWithPath(sContents, sOutPath) {
        // put sOutPath in src
        let sContentsWithPath = sContents.replace(/src="\/wp-content/gm, `src="/${sOutPath}/wp-content`);
        // put sOutPath in href
        return getRelative(sContentsWithPath, sOutPath)
    }
    ...
    requestBodyToMultipartFormData(json, boundary) {
        ...
        return encoder.encode(multipartData);
    }
...
export function getRelative(canonical, sPath) {
    // need to also deal with http:\\/\\/127.0.0.1:8000\\/ escaped??? in json
    const rc = canonical.replace(/(http|https)?:?[\/\\]+((localhost|127.0.0.1|):\d+\.?\d*[\/\\]*|[0-9a-z\-]+\.app\.github\.dev)[\/\\]?/g, sPath && sPath != '/' ? `/${sPath}/` : "/");
