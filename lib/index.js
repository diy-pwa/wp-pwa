...
ini_set('error_log', $log_file);?>`);
        ...
        if (!fs.existsSync("wp-content/themes/wpico")) {
            const oZip = new Zip({
                url: `${CORSPROXY}/github.com/mwmdev/wpico/archive/refs/heads/main.zip`,
                dest: "wp-content/themes/wpico",
                filter: "don't want to filter anything", fs
            });
            await oZip.unzip();
        }
    ...
    async getPhpInstance() {
        if (!this.php) {
            this.php = await NodePHP.load('8.3', {
