...
import { Zip, SitemapPlugin } from 'diy-pwa';
...
`);
    ...
    async installWPSitemap(){
        if (!fs.existsSync("wp-content/plugins/xml-sitemap")) {
            // only do if the sitemap plugin is not installed???
            const oSiteMapPluginZip = new Zip({fs,
                dest: `wp-content/mu-plugins/xml-sitemap`,
                url:`${CORSPROXY}/github.com/RavanH/xml-sitemap-feed/archive/refs/heads/main.zip`,
                filter:"wp-config.php"
            });
            await oSiteMapPluginZip.unzip();
            await fs.promises.writeFile(`wp-content/mu-plugins/1-xml-sitemap.php`, `<?php require_once __DIR__ . "/xml-sitemap/xml-sitemap.php";
    `);
    ...
    async setup() {
            if (!fs.existsSync(".wordpress")) {
                const oZip = new Zip({url: `${CORSPROXY}/wordpress.org/latest.zip`, 
                dest:".wordpress",
    ...
    add_filter( 'comments_open', function(){
        return false;
            ...
            !fs.existsSync("wp-content/plugins/sqlite-database-integration")) {
                const oZip = new Zip({url: `${CORSPROXY}/github.com/WordPress/sqlite-database-integration/archive/refs/heads/main.zip`, 
                dest:".wordpress",
                filter:"don't want to filter anything", fs});
                await oZip.unzip();
