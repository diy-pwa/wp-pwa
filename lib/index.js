...
`);
    ...
    async dev() {
        ...
        app.use('/', async (req, res) => {
            try {
                ...
                await this.saveNewHostIsDatabase(requestHeaders);
    ...
    async saveNewHostIsDatabase(requestHeaders){
        const sNewHost = requestHeaders['x-forwarded-host'];
        if(sNewHost){
            if(!this.requestHandler.isSaved){
                let db = await AsyncDatabase.open('./wp-content/database/.ht.sqlite');
                const sTable = await db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='wp_options'");
                if(sTable){
                    await db.exec("UPDATE wp_options SET option_value = '/%postname%/' WHERE option_name = 'permalink_structure'");
                    await db.exec(`UPDATE wp_options SET option_value = '${this.requestHandler.absoluteUrl}' WHERE option_name = 'siteurl' OR option_name = 'home'`);
                    this.requestHandler.isSaved = true;
                    await db.close();    
                    this.requestHandler.absoluteUrl = `https://${sNewHost}`;
                }
            }
        }
    }
