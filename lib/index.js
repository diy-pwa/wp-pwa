...
export default class {
    async build(sPath) {
        ...
        const app = await supertest((await this.dev(sPath)).listen());
        await this.buildSitemap(app, '/wp-sitemap.xml');
    }
    async buildSitemap(app, sSitePath){
        const response = await app.get(sSitePath);
        ...
        if(oContents.sitemapindex){
            for(const oLoc of oContents.sitemapindex.sitemap){
                const sPath = oLoc.loc.replace(/https?:\/\/.*\//,"/");
                await this.buildSitemap(app, sPath)
            }    
        // }else if(oContents.urlset){
        //     for(const oUrl of oContents.urlset){
        //         const sPath = oUrl.loc.replace(/https?:\/\/.*\//,"/");
        //         const response = await app.get(sPath);
        //         const sContents = await response.text;
        //         if (!fs.existsSync(`dist/${sPath}`)) {
        //             await fs.promises.mkdir(`dist/${sPath}`, { recursive: true });
        //         }
        //         await fs.promises.writeFile(`dist/${sPath}index.html`, sContents);
        //     }
        }
        const builder = new XMLBuilder();
        const sXML = builder.build(oContents);
        await fs.promises.writeFile(`dist/${sSitePath}`, xmlFormat(sXML));
    }
    async create() {
